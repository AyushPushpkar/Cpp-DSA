...
// Space optimized DP
int maxProfitSO(int k , vector<int>& prices) {
    int n = prices.size();
    vector<vector<int>> ahead(2, vector<int>(3, 0)), curr(2, vector<int>(k+1, 0));
...
int maxProfitMemo(int k , vector<int>& prices) {
    ...
    return stock(prices, true, k, 0, memo);
}
...
int main() {
    ...
    cout << "Max Profit (Memoized): " << maxProfitMemo(k , prices) << endl;
    cout << "Max Profit (Tabulation): " << maxProfitTab(k , prices) << endl;
    cout << "Max Profit (Space Optimized): " << maxProfitSO(k , prices) << endl;
    return 0;
}
